...
class QueryService:
    ...
    def handle_query_stream(self, request: QueryRequest) -> Iterator[str]:
        """
        Stream the answer as SSE events. Yields lines: "data: {json}\n\n".
        Events: {"type": "chunk", "content": "..."} then {"type": "done", "metadata": ..., "sources": ..., "conversation_id": ...}.
        """
        start_time = time.time()
        question = request.question.strip()
        conversation_id = request.conversation_id or f"conv_{uuid.uuid4().hex[:8]}"
        ...
        if not request.conversation_id:
            ...
            if cached_response:
                ...
                yield f"data: {json.dumps({'type': 'chunk', 'content': cached_response.answer})}\n\n"
                yield f"data: {json.dumps({'type': 'done', 'metadata': meta.model_dump(), 'sources': [s.model_dump() for s in cached_response.sources], 'conversation_id': conversation_id})}\n\n"
                return
        ...
        yield f"data: {json.dumps({'type': 'done', 'metadata': metadata.model_dump(), 'sources': [s.model_dump() for s in sources], 'conversation_id': conversation_id})}\n\n"